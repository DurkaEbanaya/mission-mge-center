...
use mc_graph_core::animation::AnimationFrame;
use mc_graph_core::dataset::DatasetGroup;
use mc_graph_core::scaling::{FillingSettings, RoundingSettings, ScalingSettings};
...
mod imp {
        ...
        fn render(&self, snapshot: &Snapshot, width: f32, height: f32, scale_factor: f64) {
            ...
            let base_snapshot = if need_redraw {
                ...
                for values in &mut data_sets {
                    crate::render::plot_group(values, &snapshot, width, height, &*object);
                }
                self.data_sets.set(data_sets);
            ...
            let Some(baze) = base_snapshot else {
                g_warning!("mc-graph-widget", "Drawing was empty");
                return;
            };
    ...
    impl ObjectSubclass for GraphWidget {
        const NAME: &'static str = "McGraphWidget";
    ...
    impl WidgetImpl for GraphWidget {
        ...
        fn snapshot(&self, snapshot: &Snapshot) {
            ...
            let native = match this.native() {
                Some(native) => native,
                None => {
                    g_critical!("mc-graph-widget", "Failed to get native");
                    return;
                }
            };
            ...
            let surface = match native.surface() {
                ...
                None => {
                    g_critical!("mc-graph-widget", "Failed to get surface");
                    return;
                }
            };
